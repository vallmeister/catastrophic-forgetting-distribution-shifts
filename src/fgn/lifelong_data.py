...
nets = {'lgl': LGL, 'afgn': AFGN, 'ktranscat': KTransCAT, 'attnktranscat': AttnKTransCAT, 'plain': PlainNet,
        'attnplain': AttnPlainNet}
...
if __name__ == "__main__":
    ...
    for batch_idx, (inputs, targets, neighbor) in enumerate(tqdm.tqdm(train_loader)):
        ...
        if args.eval and batch_idx % args.sample_rate * 10 == args.sample_rate * 10 - 1:
            ...
            with open(args.eval + '-acc.txt', 'a') as file:
                file.write(
                    (str([batch_idx * args.batch_size, test_acc, running_loss]) + '\n').replace('[', '').replace(']',
                                                                                                                 ''))
    ...
    test_acc, train_acc, valid_acc = performance(test_loader, net, args.device, k=args.k), performance(train_loader,
                                                                                                       net, args.device,
                                                                                                       k=args.k), performance(
        valid_loader, net, args.device, k=args.k)
    print("Train Acc: %.3f, Test Acc: %.3f, Valid Acc: %.3f" % (train_acc, test_acc, valid_acc))
