...
class PlainNet(nn.Module):
    ...
    Net without memory
    '''
    def __init__(self, feat_len, num_class, dropout=0.1):
        super(PlainNet, self).__init__()
        self.feat1 = FeatBrd1d(in_channels=1, out_channels=128)
        self.acvt1 = nn.Sequential(nn.BatchNorm1d(128), nn.Softsign(), nn.Dropout(dropout))
        self.feat2 = FeatBrd1d(in_channels=128, out_channels=128)
        self.acvt2 = nn.Sequential(nn.BatchNorm1d(128), nn.Softsign(), nn.Dropout(dropout))
        self.classifier = nn.Sequential(nn.Flatten(), nn.Linear(feat_len*128, num_class))
